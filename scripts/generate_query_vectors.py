...
async def main():
    ...
    if len(embeddings) != len(texts):
        print(f"❌ Embedding count mismatch: got {len(embeddings)}, expected {len(texts)}")
        sys.exit(1)
    ...
    if embeddings:
        ...
        if dim != 1024:
            print(f"⚠️  Warning: Expected 1024 dimensions (Snowflake arctic-embed-l), got {dim}")
            print(f"   Make sure you're using the same model as Page_facts collection")
