...
def update_schema_field_sets(schema_path: Path) -> dict[str, list[str]]:
    ...
    for prop in properties:
        ...
        is_evidence = (
            "evidence" in tags or 
            name.lower().endswith("evidence") or 
            "citation" in name.lower() or 
        ...
        # Check current state
        current_set = "standard" if "standard" in sets else ("extended" if "extended" in sets else None)
    ...
    with open(schema_path, "w") as f:
        yaml.dump(schema, f, Dumper=CustomDumper, default_flow_style=False, sort_keys=False, allow_unicode=True)
...
def main():
    ...
    for schema_file in sorted(schema_dir.glob("Research_*_schema.yaml")):
        ...
        if promoted or moved:
            ...
            if promoted:
                print(f"   ⬆️  Promoted to standard: {', '.join(changes['promoted_to_standard'][:5])}")
                if len(changes['promoted_to_standard']) > 5:
                    print(f"      ... and {len(changes['promoted_to_standard']) - 5} more")
            if moved:
                print(f"   ⬇️  Moved to extended: {', '.join(changes['moved_to_extended'][:5])}")
    ...
    print("  2. Commit: git add -A && git commit -m 'feat: update field sets - LLM to standard'")
